...
const authEls = {
  ...
  usernameInputLoggedIn: document.getElementById("usernameInputLoggedIn"),
  signUpBtn: document.getElementById("signUpBtn"),
  loginBtn: document.getElementById("loginBtn"),
  logoutBtn: document.getElementById("logoutBtn"),
  saveUsernameBtn: document.getElementById("saveUsernameBtn"),
  leaderboardBtn: document.getElementById("leaderboardBtn"),
  pastGamesBtn: document.getElementById("pastGamesBtn"),
  leaderboardPanel: document.getElementById("leaderboardPanel"),
  pastGamesPanel: document.getElementById("pastGamesPanel"),
  leaderboardList: document.getElementById("leaderboardList"),
  pastGamesList: document.getElementById("pastGamesList"),
  closeLeaderboardBtn: document.getElementById("closeLeaderboardBtn"),
  closePastGamesBtn: document.getElementById("closePastGamesBtn")
};
...
async function refreshAccountUI() {
  if (!supabaseClient || !authEls.accountTitle) return;

  if (!loggedInUser) {
    authEls.accountTitle.textContent = "Play as Guest";
    authEls.accountSubtitle.textContent = "Log in to save scores, appear on the leaderboard, and view past games.";
    authEls.authForms.classList.remove("hidden");
    authEls.userActions.classList.add("hidden");
    return;
  }

  authEls.authForms.classList.add("hidden");
  authEls.userActions.classList.remove("hidden");

  currentProfile = await getOrCreateProfile();
  const username = currentProfile?.username || loggedInUser.email?.split("@")[0] || "Player";
  authEls.accountTitle.textContent = `Logged in as ${username}`;
  authEls.accountSubtitle.textContent = "Scores save automatically when you finish a Perfect Day.";
  authEls.usernameInputLoggedIn.value = username;
}
...
async function getOrCreateProfile() {
  ...
  const starterUsername = cleanUsername(authEls.usernameInput.value || loggedInUser.email?.split("@")[0] || `player_${loggedInUser.id.slice(0, 6)}`);
...
async function signUpWithEmail() {
  const email = authEls.emailInput.value.trim();
  const password = authEls.passwordInput.value;
  const username = cleanUsername(authEls.usernameInput.value);
  ...
  if (data.user) {
    ...
    if (profileError) alert(`Account created, but username did not save: ${profileError.message}`);
    else alert("Account created. If Supabase email confirmation is on, check your email before logging in.");
...
async function saveUsername() {
  if (!loggedInUser) return alert("Log in first.");
  const username = cleanUsername(authEls.usernameInputLoggedIn.value);
...
function gameSignature() {
  return JSON.stringify({ mode: els.mode.value, build });
}
...
async function loadLeaderboard() {
  if (!supabaseClient) return alert("Supabase is not loaded yet.");
  authEls.leaderboardPanel.classList.remove("hidden");
  authEls.leaderboardList.innerHTML = "Loading leaderboard...";
  ...
  if (error) {
    authEls.leaderboardList.textContent = error.message;
    return;
  }

  if (!data || data.length === 0) {
    authEls.leaderboardList.textContent = "No scores yet. Finish a game while logged in.";
    return;
  }

  authEls.leaderboardList.innerHTML = data.map((row, index) => `
    <div class="data-row">
...
async function loadPastGames() {
  authEls.pastGamesPanel.classList.remove("hidden");

  if (!loggedInUser) {
    authEls.pastGamesList.textContent = "Log in to view past games.";
    return;
  }

  authEls.pastGamesList.innerHTML = "Loading past games...";
  ...
  if (error) {
    authEls.pastGamesList.textContent = error.message;
    return;
  }

  if (!data || data.length === 0) {
    authEls.pastGamesList.textContent = "No past games yet.";
    return;
  }

  authEls.pastGamesList.innerHTML = data.map(row => {
    const cities = Object.values(row.game_data || {})
      .map(item => item.city)
      .filter(Boolean)
      .slice(0, 3)
      .join(" · ");
    ...
    return `
      <div class="data-row">
        <div class="data-rank">${row.score}</div>
        <div>
          <div class="data-main">${row.mode || "Everyday"} — ${row.score_tier || "Completed"}</div>
          <div class="data-sub">${new Date(row.created_at).toLocaleString()}${cities ? ` · ${cities}` : ""}</div>
        </div>
        <div class="data-score">${row.score}/100</div>
      </div>
    `;
...
function bindSupabaseButtons() {
  if (!authEls.signUpBtn) return;
  authEls.signUpBtn.addEventListener("click", signUpWithEmail);
  authEls.loginBtn.addEventListener("click", loginWithEmail);
  authEls.logoutBtn.addEventListener("click", logout);
  authEls.saveUsernameBtn.addEventListener("click", saveUsername);
  authEls.leaderboardBtn.addEventListener("click", loadLeaderboard);
  authEls.pastGamesBtn.addEventListener("click", loadPastGames);
  authEls.closeLeaderboardBtn.addEventListener("click", () => authEls.leaderboardPanel.classList.add("hidden"));
  authEls.closePastGamesBtn.addEventListener("click", () => authEls.pastGamesPanel.classList.add("hidden"));
}
